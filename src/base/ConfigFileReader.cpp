...
CConfigFileReader * CConfigFileReader::getInstance()
{
	static CConfigFileReader instance;
	return &instance;
}
...
int CConfigFileReader::initConfigName(const char * file)
{ 
...
int CConfigFileReader::_LoadFile(const char* filename)
{
    m_config_file.clear();
	...
	if (!fp)
	{
		LogMsg(LOG_ERROR, "open file %s fiail", filename);
		return IM_ERR;
	}
...
char* CConfigFileReader::_TrimSpace(char* name)
{
	...
	while ((*end_pos == ' ') || (*end_pos == '\t') || (*end_pos == '\r') || (*end_pos == '\n'))
	{
		*end_pos = 0;
		end_pos--;
	}
